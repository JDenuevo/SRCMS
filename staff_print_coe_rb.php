...
<body>
            ...
            <div id="print-logo">
                ...
                <div class = "btnback">
                    <a href="staff_report_rb.php"><button>BACK</button>
                    </a>
                </div>
...
<?php
    ...
    {
        $per_id = $_GET['per_id'];
        $query = "SELECT * FROM srcms_contract_rb WHERE per_id='$per_id' ORDER BY cttime ASC";
    ...
    {
        $per_id = $_GET['per_id'];
        $query = "SELECT * FROM srcms_contract_rb WHERE per_id='$per_id' ORDER BY cont_id ASC";
        if($rs=$conn->query($query)){
           while($row=$rs->fetch_assoc()){
