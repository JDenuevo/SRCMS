...
session_start();
include ("db_conn.php");
if (isset($_SESSION['id']) && isset($_SESSION['user_name']) && isset($_SESSION['user_name'])) {
...
<title>Admin Find Departments | SR&CMS</title>
    ...
    <div id="mid-search">
      <form method="GET">
        <label>Search Departments <label>
          <input type="text" name="name" placeholder="Search for Departments" value="<?php if(isset($_GET['name'])){echo $_GET['name'];} ?>" required>
...
<div id="print">
    ...
    <br> <br>
    <h2 class = "text-center">DEPARTMENT'S BREAKDOWN</h2>
        <div class="table-responsive">
            <table id = "table" class = "table table-hover table-bordered table-striped m-auto text-dark">
              <thead>
                ...
                if(isset($_GET['name'])){
                    $name = $_GET['name'];
                    $result = "SELECT * FROM srcms_tbl_personnel WHERE CONCAT(department) LIKE '%$name%'";
