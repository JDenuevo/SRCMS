...
<title>Update Account | SR&CMS</title>
...
{
    $id = $_GET['id'];
    $query = "SELECT * FROM srcms_users WHERE id='$id'";
	if($rs=$conn->query($query)){
        if($rs->num_rows>0){
           ...
           while ($uquery=$rs->fetch_assoc()) {
                ...
                ?>
<form action="adminmanageacc_edit_query.php" method="POST" enctype="multipart/form-data" name="form1" id="form1">
<br><br>
	...
	<section class = "p-2 text-center">   
		<div class = "container">          
			...
			<h1><?php if(isset($_SESSION['msg'])){echo $_SESSION['msg']; unset($_SESSION['msg']);}?></h1>
				...
				<h2 class = "text-center fw-bold">UPDATE YOUR ACCOUNT</h2>
				<p class = "text-center text-muted lead">Make sure to check twice.</p>
				<div class="row row-cols-2">
					<div class = "col-lg-6 col-md-6 col-sm-12">
							<label>Username</label>
							<input type="text" name="user_name" class="form-control" placeholder="Username" value="<?= $uquery['user_name']  ?>">
						</div>
						<div class = "col-lg-6 col-md-6 col-sm-12">
							<label>Current Password</label>
							<input type="password" name="pass" class="form-control" placeholder="Current Password" value="<?= $uquery['pass']  ?>" readonly>
						</div>	
					</div> 
					<br>
					<div class="row row-cols-2">
						...
						<div class = "col-lg-6 col-md-6 col-sm-12">
							...
							<input type="password"  name="repass" class="form-control" placeholder="Re-Type New Password">
						</div>
					</div>
					<br>
					<div class="row row-cols-2">
						<div class = "col-lg-6 col-md-6 col-sm-12">
							<label>Name</label>
							<input type="text" name="name" class="form-control" placeholder="Name" value="<?= $uquery['name']  ?>">
						</div>
						<div class = "col-lg-6 col-md-6 col-sm-12">
							<label>User Type</label>
							<select name="user_type" class="form-control" placeholder="User Type" value="<?= $uquery['user_type']  ?>">
					...
					<a href="adminmanageacc.php">
						<button class = "btn btn-success btn-lg" id = "btnupd" type="submit" name="update">
							<img class = "img-fluid" id = "navicons" src = "photos/updacc.png">
							UPDATE ACCOUNT
						</button>
                	</a>
			</div>
