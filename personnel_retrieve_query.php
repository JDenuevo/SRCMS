...
if(isset($_POST['retrieve'])) {
	...
	$sql1 = "INSERT INTO srcms_tbl_personnel (per_id, per_firstname, per_middlename, per_lastname, per_suffix, per_gender, per_status, per_address, per_date_of_birth, per_place_of_birth, per_tin_no, per_gsis_bp_no, per_email, per_contact_no, bs_name, bs_year, bs_school, ms_name, ms_year, ms_school, dr_name, dr_year, dr_school) 
    VALUES ('$per_id','$per_firstname', '$per_middlename', '$per_lastname','$per_suffix', '$per_gender','$per_status','$per_address', '$per_date_of_birth', '$per_place_of_birth', '$per_tin_no', '$per_gsis_no', '$per_email', '$per_contact_no', '$bs_name', '$bs_year', '$bs_school', '$ms_name', '$ms_year', '$ms_school', '$dr_name', '$dr_year', '$dr_school')";
     if($conn->query($sql1)){
		$sql = "DELETE FROM srcms_tbl_personnel_rb WHERE per_id = '$per_id'";
		if($conn->query($sql)){
			$sql2 = "INSERT INTO srcms_contract (cont_id, per_id, cttime, ctdue, position, department, colleges, status, salary, campus, semester, schoolyear) SELECT cont_id, per_id, cttime, ctdue, position, department, colleges, status, salary, campus, semester, schoolyear FROM srcms_contract_rb WHERE per_id = '$per_id' ";
		if($conn->query($sql2)){
		$sql3 = "DELETE FROM srcms_contract_rb WHERE per_id = '$per_id'";
		$conn->query($sql3);
	header("Location: admin_proflist.php?Record_Retrieved");
				}   
    ...
    header("Location: admin_proflist.php?Record_NOT_Retrieved");
     }
?>